...
將編寫好的新詞典命名爲 `luna_pinyin.dict.yaml` 放置於 Rime 的 [用戶文件夾](https://github.com/rime/home/wiki/UserData)，重新部署即可。
...
若個別編碼的格式與主詞典的編碼類型（例如拼音）不同，推薦採用自定義短語機制。

自定義短語是一種用文本文件存儲的簡單 Rime 詞典。可定義編碼爲固定字符串的文字。

預設的「朙月拼音」輸入方案已經包含自定義短語的相關設置，文件位置爲
[用戶文件夾](https://github.com/rime/home/wiki/UserData)/`custom_phrase.txt`，
按下文給出的樣例創建文本文件，並重新部署即可。

用家可參考「朙月拼音」在自己的方案中添加兩處代碼，以啓用自定義短語：
...
Rime 自定義短語文件樣例 <https://gist.github.com/lotem/5440677>
...
#
# 碼表各字段以製表符（Tab）分隔。各字段爲：
#   - 文字
#   - 編碼
#   - 權重（決定重碼的次序、可選，權重高則選項在前）
#
# 雖然文本碼表方便編輯，但性能有限，不適合導入大量條目。
#
...
默認地，詞典快照備份到 [RIME用戶文件夾](https://github.com/rime/home/wiki/UserData)\sync\UUID 這個地方。如果你要用 Dropbox 或 U盤在不同機器/系統之間同步用戶詞典，則需要設定同步的目標文件夾，如 `D:\Dropbox\RimeSync` 。
